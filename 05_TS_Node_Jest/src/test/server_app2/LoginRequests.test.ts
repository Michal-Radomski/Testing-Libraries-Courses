...
describe("Login requests", (): void => {
  ...
  it("should not login user with invalid credentials", async (): Promise<void> => {
    requestWrapper.method = HTTP_METHODS.POST;
    requestWrapper.body = someAccount;
    requestWrapper.url = "localhost:8080/login";
  ...
  it("should return bad request if no credentials in request", async (): Promise<void> => {
    requestWrapper.method = HTTP_METHODS.POST;
    requestWrapper.body = {};
    requestWrapper.url = "localhost:8080/login";
